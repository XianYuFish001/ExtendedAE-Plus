...
public class ProviderSelectScreen extends Screen {
        ...
        try {
            ...
            if (recent != null && !recent.isEmpty()) {
                ...
                recent.forEach(q -> {
                    ...
                    if (alias != null && !query.contains(alias)) query.add(0, alias);
                    else query.add(q);
                });
                this.selectedQuery = query.get(0);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == 1 && this.searchBox != null) {
            if (Minecraft.getInstance().player != null)
                Minecraft.getInstance().player.playSound(SoundEvents.UI_BUTTON_CLICK.get(), 0.1F, 1.0F);
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double delta) {
        ...
        if (mouseX >= boxX && mouseX <= boxX + boxWidth &&
            ...
            if (direction && index == 0) selectedQuery = query.get(query.size() - 1);
            else if (direction) selectedQuery = query.get(index - 1);
            else if (index == query.size() - 1) selectedQuery = query.get(0);
