...
import com.extendedae_plus.util.keyBuilder.toKeyPattern
...
class LangZH(output: PackOutput) : LanguageProvider(output, ExtendedAEPlus.MODID, "zh_cn") {
    override fun addTranslations() {
        ...
        this.addItem(ModItems.CellInfinity, "吞噬万籁的寂静")
        this.addItem(ModItems.ControllerProvider, "样板供应器管理工具")
        this.addItem(ModItems.CardChannel, "频道卡")
        this.addItem(ModItems.CardAutoCompletion, "自动完成卡")
        this.addItem(ModItems.PriorityTool, "优先级覆写卡")
        this.addItem(ModItems.CardTicking, "Ticking倍增卡")
        UtilKeyBuilder.dataGen(ModItems.CardTicking)
            .addStr("multiplier")
            .buildInto("Ticking倍增卡 %s倍")

        this.addBlock(ModBlocks.WirelessTransceiver, "无线收发器")
        this.addBlock(ModBlocks.PortUpload, "装配矩阵上传接口")
        this.addBlock(ModBlocks.CoreAdvancedCrafter, "装配矩阵高级合成核心")
        this.addBlock(ModBlocks.CoreAdvancedPattern, "装配矩阵高级样板核心")
        this.addBlock(ModBlocks.CoreAdvancedSpeed, "装配矩阵高级速度核心")
